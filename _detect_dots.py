...
# Filter dots that fall outside the clean-map bounds (fluke detections / edge effects)
clean = np.array(Image.open(CLEAN).size)  # (W, H)
inside = (mapped[:, 0] >= 50) & (mapped[:, 0] <= clean[0] - 50) & \
         (mapped[:, 1] >= 50) & (mapped[:, 1] <= clean[1] - 50)
mapped = mapped[inside]
print(f"Dots inside clean map: {len(mapped)} of {len(centroids)}")
